	...
	public function getFormDescriptor(
		...
		if ( $data !== null ) {
			...
			if ( count( $respArray ) != 0 ) {
				foreach ( $respArray as $resp ) {
					$responders[] = $userLinkRenderer->userLink( $userFactory->newFromName( $resp ), $context );
				}
			}
		}
		...
		if ( $id ) {
			...
			foreach ( $dbReviewers as $db ) {
				if ( $db->r_timestamp ) {
					$reviewers['reviewed'][] = $userLinkRenderer->userLink( $userFactory->newFromName( $db->r_user ), $context );
				} else {
					$reviewers['unreviewed'][] = $userLinkRenderer->userLink( $userFactory->newFromName( $db->r_user ), $context );
				}
